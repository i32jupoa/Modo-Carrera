...
import { sortUCLTable, UCLTableEntry, UCLBracketSlot } from "@/data/ucl";
import type { Fixture } from "@/lib/season";

export const Route = createFileRoute("/conference-league")({ component: EuropeanCompetitionPage });
                  ...
                  <tr key="cut-playoff">
                    <td colSpan={11} className="py-0 px-0">
                      <div className="flex items-center gap-2 px-3 py-0.5 bg-yellow-950/30">
                        ...
                        <span className="text-[0.6rem] text-yellow-500 font-semibold uppercase tracking-wider whitespace-nowrap">
                          Play-offs
                        </span>
                        <div className="flex-1 h-px bg-yellow-600/40" />
                      </div>
                  ...
                  <tr key="cut-elim">
                    <td colSpan={11} className="py-0 px-0">
                      <div className="flex items-center gap-2 px-3 py-0.5 bg-red-950/30">
                        ...
                        <span className="text-[0.6rem] text-red-400 font-semibold uppercase tracking-wider whitespace-nowrap">
                          Eliminados
                        </span>
                        <div className="flex-1 h-px bg-red-600/40" />
                      </div>
                ...
                >
                  ...
                  <td className="py-2 pr-3">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className={`font-semibold truncate ${isUser ? "text-primary" : ""}`}>
                        {teamName(e.teamId)}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="py-2 text-center tabular-nums text-destructive font-semibold">
                    {e.lost}
                  </td>
                  <td className="py-2 text-center tabular-nums">{e.gf}</td>
                  <td className="py-2 text-center tabular-nums">{e.ga}</td>
                  <td
                    className={`py-2 text-center tabular-nums font-semibold ${e.gd > 0 ? "text-emerald-400" : e.gd < 0 ? "text-destructive" : ""}`}
                  >
                    {e.gd > 0 ? `+${e.gd}` : e.gd}
                  </td>
                  <td className="py-2 pr-2 text-center tabular-nums font-black text-primary text-sm">
                    {e.points}
                  </td>
                  <td className="py-2 pr-3">
...
function FixtureRow({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 hover:bg-muted/30 text-sm ${isUser ? resultBg || "bg-green-950/20 border-l-2 border-green-400" : ""} ${played ? "cursor-pointer" : ""}`}
      onClick={() => played && onClick?.(f)}
    >
      <div className="flex-1 flex items-center justify-end gap-2 min-w-0">
        <span
          className={`text-right truncate max-w-[130px] ${f.homeId === myTeamId ? "font-bold text-white" : ""}`}
        >
          {teamName(f.homeId)}
        </span>
        <Logo id={f.homeId} />
      ...
      <div className="flex-1 flex items-center gap-2 min-w-0">
        <Logo id={f.awayId} />
        <span
          className={`truncate max-w-[130px] ${f.awayId === myTeamId ? "font-bold text-white" : ""}`}
        >
          {teamName(f.awayId)}
        </span>
      </div>
  ...
  if (finalFixture) {
    final = {
      ...
      winner: finalFixture.result
        ? finalFixture.result.homeGoals > finalFixture.result.awayGoals
          ? finalFixture.homeId
          : finalFixture.awayId
        : null,
    };
...
function getRealMatchResult(
  ...
  if (matchup.round === "final") {
    ...
    if (finalMatch?.result) {
      ...
      const penalties = finalMatch.result.penalties
        ? {
            homeGoals: finalMatch.result.penalties.homeGoals,
            awayGoals: finalMatch.result.penalties.awayGoals,
          }
        : undefined;
      return {
  ...
  const leg1 = roundFixtures.find(
    (f) => f.homeId === matchup.homeTeam && f.awayId === matchup.awayTeam,
  );
  const leg2 = roundFixtures.find(
    (f) => f.homeId === matchup.awayTeam && f.awayId === matchup.homeTeam,
  );

  if (leg1?.result && leg2?.result) {
    // Check if leg2 went to extra time or penalties
    const extraTime = !!leg2.result.extraTime;
    const penalties = leg2.result.penalties
      ? { homeGoals: leg2.result.penalties.homeGoals, awayGoals: leg2.result.penalties.awayGoals }
      : undefined;
    ...
    if (penalties) {
      ...
      const homeAgg =
        leg1.result.homeGoals + leg2.result.awayGoals + (leg2.result.extraTime?.awayGoals || 0);
      const awayAgg =
        leg1.result.awayGoals + leg2.result.homeGoals + (leg2.result.extraTime?.homeGoals || 0);
...
function BracketView({
  ...
  return (
    <div
      className="w-full min-h-screen p-8 relative overflow-hidden"
      style={{
        ...
        <div className="flex items-center justify-between gap-6 min-h-[900px] py-8">
          ...
          <div className="flex-1 flex items-center gap-4">
            ...
            <div className="flex-1 flex flex-col justify-around gap-8">
              <div className="text-center text-sm font-bold text-green-400 uppercase tracking-wider mb-4">
                Play-off
              </div>
          ...
          <div className="w-48 flex flex-col items-center justify-center gap-6">
            ...
            {bracketState.final.homeTeam && bracketState.final.awayTeam && (
              <div className="flex items-center gap-4 mb-4">
                ...
                {/* Trophy */}
                <div className="relative flex h-24 w-24 items-center justify-center rounded-3xl bg-black/20 ring-1 ring-white/10">
                  <Trophy className="h-14 w-14 drop-shadow-2xl" style={{ color: config.primary, filter: `drop-shadow(0 0 18px ${config.primary}66)` }} />
                </div>
...
function CompactMatchCard({
  ...
  getMatchResult: (
    slot: UCLBracketSlot,
  ) => { homeGoals: number; awayGoals: number; winner: string } | null;
...
function DarkMatchCard({
  ...
  safeTeamName: (id: string) => string;
  getTeamLogoPath: (id: string) => string;
  getMatchResult: (slot: UCLBracketSlot) => {
    homeGoals: number;
    awayGoals: number;
    winner: string;
    extraTime?: boolean;
  ...
  return (
    <div className="bg-[#111827] rounded-xl border border-cyan-500/30 overflow-hidden">
      ...
      {/* Final date/time */}
      {isFinal && (
        <div className="px-3 py-1 bg-[#0f172a] text-gray-400 text-xs text-center">{date}</div>
      )}
        ...
        <div className="space-y-4">
          {tieArray.map((tieFixtures, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-[#0b0f19] rounded-xl border border-gray-800 overflow-hidden"
              >
                {/* Aggregate Score Row */}
                {aggregate && (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex-1 flex flex-col items-center">
                        ...
                        <div className="flex items-center justify-between w-full max-w-xs">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-gray-300 text-xs">
                              {safeTeamName(leg2.awayId)}
                            </span>
                            {awayLogo2 ? (
                              <img
                                src={awayLogo2}
                                alt={safeTeamName(leg2.awayId)}
                                className="w-4 h-4 object-contain"
                                onError={(e) => {
                                  e.currentTarget.style.display = "none";
                                }}
...
function EuropeanCompetitionPage() {
  const [save, setSave] = useState<SaveGame | null>(null);
  const competition = "uecl" as const;
  const config = EUROPEAN_CONFIGS[competition];
  ...
  useEffect(() => {
    let raw = loadSave();
    if (!raw) { setSave(raw); return; }
    if (!raw[competition]) {
      raw = initializeEuropeanCompetition(raw, competition);
      saveSave(raw);
    }
    if (!raw[competition]?.drawState.leagueDone) {
      setSave(raw);
      return;
    }
    const offset = uclDayOffset(currentDate);
    const synced = simulatePendingEuropeanThroughDay(raw, competition, offset, raw.myTeamId);
    saveSave(synced);
    setSave(synced);
  }, [currentDate]);
  ...
  const ucl = save[competition];
  const fixtures = save[competition === "uel" ? "uelFixtures" : "ueclFixtures"] ?? [];
  ...
  if (!ucl || !ucl.phase || !ucl.participants) {
    return (
      <div className="p-6 text-center space-y-2">
        ...
        <h2 className="text-xl font-bold">UEFA Conference League</h2>
        <p className="text-muted-foreground">
          UEFA Conference League comparte el formato de la Champions: sorteo, fase de liga, play-offs, eliminatorias y final.
        </p>
      </div>
    );
  }
  ...
  const displayTable = ucl.phase === "league" ? (ucl.table ?? []) : (ucl.leaguePhaseTable ?? ucl.table ?? []);
  ...
  const otherFixtures = fixtures.filter(
    (f) => f.round && !f.round.startsWith("Jornada") && f.matchday > 0,
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-950 to-green-950 border-b border-border px-4 py-5">
        <div className="flex items-center gap-3">
          <div className="text-3xl">
            <Trophy className="h-8 w-8 text-white/90" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">UEFA Conference League</h1>
      ...
      <div className="p-4">
        ...
        {tab === "partidos" &&
          (() => {
            // Build phase buckets
            const playoffFx = fixtures.filter(
              (f) => f.round?.startsWith("Playoff") && f.matchday > 0,
            );
            ...
            const activePhase =
              phaseBuckets.find((b) => b.key === phaseTab) ?? phaseBuckets[phaseBuckets.length - 1];
                            ...
                            (() => {
                              ...
                              return (
                                <div className="rounded-lg border border-border overflow-hidden">
                                  <div className="bg-green-950/60 px-4 py-2.5 flex items-center justify-between">
                                    <div>
                                      <span className="text-sm font-bold text-green-200">
                                        {activeRound}
                                      </span>
                                      <span className="ml-2 text-xs text-green-400">
                                        {europeanFixtureDate(matchday)}
                                      </span>
                                    </div>
                                    <span className="text-xs text-green-400">
                                      {played}/{fxs.length} jugados
                                    </span>
                                  </div>
