...
import { selectTopScorers } from "@/store/playersStore";
...
function ScorersPage() {
  ...
  const scorers = useMemo(
    () =>
      save && ready
        ? selectTopScorers(league === "all" ? undefined : league, 30)
        : [],
    [save, ready, league],
  );
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      {scorers.length === 0 ? (
        <p className="text-sm text-muted-foreground text-center py-12">Aún no hay goles registrados. ¡Juega tu primer partido!</p>
      ) : (
