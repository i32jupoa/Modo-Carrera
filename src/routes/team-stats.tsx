...
import { Trophy, Star, ShieldCheck, Activity, Goal, Sparkles } from "lucide-react";
...
function statusForPlayer(stats: PlayerStats, save: ReturnType<typeof loadSave>, playerId: string) {
  if (!save) return "-";
  const suspensions = save.suspensions?.[save.myTeamId] ?? [];
  const suspension = suspensions.find((s) => s.playerId === playerId);
  if (suspension && suspension.matchdaysRemaining > 0) return `S${suspension.matchdaysRemaining}j`;
  if (stats.injuredUntil > 0) return `I${stats.injuredUntil}j`;
  return "-";
}
  ...
  return (
    <div className="mx-auto max-w-7xl p-4 md:p-6">
      <div className="panel-glow mb-6 overflow-hidden">
        <div className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
            ...
            <StatPill label="PJ" value={String(totals.appearances)} />
                ...
                <table className="w-full min-w-[1080px] text-sm">
                  ...
                  <tbody>
                    {players.map((p) => {
                      ...
                      const status = statusForPlayer(st, save, String(p.ID));
                      ...
                      return (
                        <tr key={p.ID} className="border-b border-border/30 hover:bg-secondary/20">
                          ...
                          <td className="px-2 py-2 text-center font-semibold">{status}</td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      <p className="mt-4 text-[0.65rem] text-muted-foreground">
        Ordenado por posición · la <strong>Nota media</strong> solo contabiliza partidos que el jugador ha disputado y tienen valoración registrada. P0 = porterías a cero; MVP = mejor jugador del partido; Estado: S = sancionado, I = lesionado.
      </p>
    </div>
  );
}
