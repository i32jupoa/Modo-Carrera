...
function FixturesPage() {
  ...
  useEffect(() => {
    ...
    setSave(s);
    setLeague(lg);
    setViewMd(s.currentMatchday[lg] ?? 1);
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
          ...
          <DropdownMenuTrigger asChild>
            <button className="bg-secondary border border-border rounded px-3 py-1.5 text-sm flex items-center gap-2 hover:border-primary/60 transition">
              <LeagueLogo
                ...
                size="sm"
                fallback={<span className="text-xs">{LEAGUES[league].flag}</span>}
              />
              {LEAGUES[league].name}
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={() => setViewMd((m) => Math.max(1, m - 1))}
          disabled={viewMd <= 1}
          className="px-3 py-1.5 rounded bg-card border border-border text-sm disabled:opacity-30"
        >
          ←
        </button>
        ...
        <h2 className="text-2xl font-bold">
          Jornada {viewMd}{" "}
          <span className="text-muted-foreground text-base font-normal">/ {totalMd}</span>
        </h2>

        <button
          onClick={() => setViewMd((m) => Math.min(totalMd, m + 1))}
          disabled={viewMd >= totalMd}
          className="px-3 py-1.5 rounded bg-card border border-border text-sm disabled:opacity-30"
        >
          →
        </button>
      ...
      <div className="panel divide-y divide-border/40">
        {fixtures.map((f) => {
          ...
          const isFocus =
            !!focusTeam && (f.homeId === focusTeam || f.awayId === focusTeam) && !isMine;
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 justify-end min-w-0">
                <span
                  className={`font-semibold truncate ${isMine && f.homeId === save.myTeamId ? "text-primary" : ""}`}
                >
                  {home.name}
                </span>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span
                  className={`font-semibold truncate ${isMine && f.awayId === save.myTeamId ? "text-primary" : ""}`}
                >
                  {away.name}
                </span>
              </div>
