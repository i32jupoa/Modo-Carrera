...
function inRegion(country: string, region: Region): boolean {
  ...
  switch (region) {
    case "europe":
      return lon >= -12 && lon <= 45 && lat >= 35 && lat <= 72;
    case "northam":
      return lon >= -170 && lon <= -50 && lat >= 12 && lat <= 75;
    case "southam":
      return lon >= -90 && lon <= -30 && lat >= -56 && lat <= 12;
    case "asia":
      return lon >= 45 && lon <= 150 && lat >= -10 && lat <= 60;
    case "oceania":
      return lon >= 110 && lon <= 180 && lat >= -50 && lat <= 0;
  }
}
