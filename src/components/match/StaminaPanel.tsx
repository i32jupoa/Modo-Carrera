...
export function StaminaBar({ value }: { value: number }) {
  ...
  const color =
    tone === "ok" ? "bg-primary" : tone === "warn" ? "bg-yellow-400" : "bg-destructive";
  return (
            ...
            <div key={p.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="shrink-0 text-[0.6rem] font-bold text-muted-foreground w-8">{p.position}</span>
                  <span className="truncate text-xs font-semibold">{p.name}</span>
                </div>
                <div className="mt-1"><StaminaBar value={st} /></div>
              </div>
