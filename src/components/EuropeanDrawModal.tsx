...
import {
  SaveGame,
  applyEuropeanLeagueDraw,
  applyEuropeanPlayoffDraw,
  applyEuropeanKnockoutDraw,
} from "@/lib/store";
import { assignUCLPots, emptyTableEntry } from "@/data/ucl";
import { EUROPEAN_CONFIGS, type EuropeanStateKey } from "@/data/europeanCompetitions";
...
interface EuropeanDrawModalProps {
  type: "league" | "playoff" | "knockout";
  competition: EuropeanStateKey;
  save: SaveGame;
  onClose: () => void;
  onComplete: (updated: SaveGame) => void;
}

export function EuropeanDrawModal({ type, competition, save, onClose, onComplete }: EuropeanDrawModalProps) {
  const config = EUROPEAN_CONFIGS[competition];
  const participantIds = config.participants;
  const fixtureKey = competition === "uel" ? "uelFixtures" : "ueclFixtures";
  ...
  const getTeamOpponents = () => {
    ...
    const introPots = assignUCLPots(participantIds);

    // Re-run draw to get assignments with correct home/away info
    const { assignments } = runSwissDraw(participantIds);
  ...
  function ensureEuropean(s: SaveGame): SaveGame {
    const participants = participantIds;
    return {
      ...s,
      [competition]: {
        phase: s[competition]?.phase ?? ("league" as const),
        seasonNumber: s[competition]?.seasonNumber ?? 1,
        participants,
        table: s[competition]?.table?.length === 36 ? s[competition].table : participants.map(emptyTableEntry),
        drawState: s[competition]?.drawState ?? {
          leagueDone: false,
          playoffDone: false,
          knockoutDone: false,
        },
        leaguePhaseTable: s[competition]?.leaguePhaseTable ?? null,
        bracket: s[competition]?.bracket ?? [],
      },
    };
  }

  useEffect(() => {
    if (type === "playoff" && save[competition]?.drawState.playoffDone) {
      setResult(ensureEuropean(save));
      setMyFixtures([]);
      setStep("done");
    }
  }, [type, save]);
  ...
  function runDraw() {
    const currentSave = ensureEuropean(save);
    ...
    try {
      ...
      if (type === "league") {
        ...
        const drawResult = runSwissDraw(participantIds);
        ...
        updated = applyEuropeanLeagueDraw(currentSave, competition, drawResult);
        const pots = assignUCLPots(participantIds);
        setDrawPots(pots);
        const potOf = new Map<string, number>();
        pots.forEach((pot, i) => pot.forEach((id) => potOf.set(id, i)));
        fixtures = updated[fixtureKey].filter(
        ...
        updated = currentSave[competition]?.drawState.playoffDone
          ? currentSave
          : applyEuropeanPlayoffDraw(currentSave, competition);
        fixtures = updated[fixtureKey].filter(
          (f) =>
            (f.homeId === myTeamId || f.awayId === myTeamId) &&
            (f.round === "Playoff-Leg1" || f.round === "Playoff-Leg2"),
        );
      } else {
        updated = applyEuropeanKnockoutDraw(currentSave, competition);
        fixtures = updated[fixtureKey].filter(
  ...
  const titles = {
    league: `Sorteo · Fase de Liga ${config.shortName}`,
    playoff: `Sorteo · Play-offs ${config.shortName}`,
    knockout: `Sorteo · Octavos de Final ${config.shortName}`,
  };
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2">
        <div
          className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-5xl flex flex-col"
          style={{ height: "calc(100vh - 2rem)" }}
        >
          {/* Header */}
          <div className="px-6 py-3 rounded-t-xl flex items-center gap-3 shrink-0" style={{ background: `linear-gradient(90deg, ${config.primary}, #09090b 85%)` }}>
          ...
          <div className="flex-1 overflow-hidden flex gap-0 min-h-0">
              ...
              <div className="w-72 shrink-0 border-r border-border overflow-y-auto p-3 space-y-2 bg-black/10">
                ...
                {potsToShow.map((pot, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`rounded-lg border bg-gradient-to-b p-2 ${POT_COLORS[i]}`}
                    >
                      <div
                        className={`text-[0.65rem] font-bold uppercase tracking-widest mb-1.5 ${POT_TEXT[i]}`}
                      >
                        Bombo {i + 1}
                      </div>
                        ...
                        {pot.map((id) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {isMyTeam && (
                                <span className="ml-auto text-[0.55rem] text-blue-300 font-bold shrink-0">
                                  TÚ
                                </span>
                              )}
                              {isRival && (
                                <span className="ml-auto text-[0.55rem] text-green-400 font-bold shrink-0">
                                  RIVAL
                                </span>
                              )}
                            </span>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 min-h-0">
              ...
              {(step === "revealing" || step === "done") && (
                <>
                  <p className="text-sm font-semibold shrink-0" style={{ color: config.primary }}>
                    {type === "league"
                      ? `Tus rivales — ${visibleCount} / ${myFixtures.length} revelados`
                      : type === "playoff"
                        ? "Tu eliminatoria de play-off:"
                        : "Tu eliminatoria de octavos:"}
                  </p>
                    ...
                    <div className="space-y-4">
                      {[0, 1, 2, 3].map((potIdx) => {
                        ...
                        return (
                          <div key={potIdx}>
                            <div
                              className={`text-xs font-bold uppercase tracking-widest mb-2 ${POT_TEXT[potIdx]}`}
                            >
                              Bombo {potIdx + 1}
                            </div>
                                  ...
                                  >
                                    <div className="flex items-center gap-2 bg-muted/40 border border-border rounded-xl px-3 py-2.5">
                                      <div className="flex flex-col items-center gap-0.5 flex-1">
                                        ...
                                        {isHome && (
                                          <span className="text-[0.55rem] font-bold" style={{ color: config.secondary }}>
                                            TÚ
                                          </span>
                                        )}
                                      </div>
                          ...
                          >
                            <div className="flex items-center gap-3 bg-muted/40 border border-border rounded-xl px-4 py-3">
                              <div className="flex flex-col items-center gap-1 flex-1">
                                ...
                                <span className="text-xs font-semibold text-center max-w-[90px] truncate">
                                  {teamName(isHome ? save.myTeamId : rivalId)}
                                </span>
                                {isHome && (
                                  <span className="text-[0.6rem] font-bold" style={{ color: config.secondary }}>TÚ</span>
                                )}
                              </div>
                              <div className="flex flex-col items-center shrink-0">
          ...
          <div className="px-5 py-3 border-t border-border flex justify-between items-center shrink-0">
            ...
            <div className="flex gap-2">
              ...
              {step === "revealing" && visibleCount < myFixtures.length && (
                <button
                  onClick={revealNext}
                  className="px-5 py-2 rounded text-white text-sm font-bold hover:brightness-110 transition" style={{ backgroundColor: config.primary }}
                >
                  Revelar siguiente →
                </button>
              )}
      ...
      {showMatrixModal && drawMatrix && drawTeamIndex && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col">
            {/* Header */}
            <div className="px-6 py-4 rounded-t-xl flex items-center justify-between shrink-0" style={{ background: `linear-gradient(90deg, ${config.primary}, #09090b 85%)` }}>
              <div>
                <h2 className="text-white font-bold text-lg">Matriz de Emparejamientos</h2>
            ...
            <div className="flex-1 overflow-auto p-4 bg-gradient-to-br from-slate-900 via-blue-950 to-indigo-950">
              <div className="grid grid-cols-2 gap-4 h-full">
                {teamOpponents.map((pot, potIndex) => (
                  <div
                    key={potIndex}
                    className="bg-black/30 rounded-lg p-3 border border-blue-900/50"
                  >
                    <h3 className="font-bold text-sm mb-3 text-center" style={{ color: config.secondary }}>
                      Bombo {potIndex + 1}
                    </h3>
                    <div className="space-y-2">
